...
function Container(props) {
  const { name, badgeCount, color, size, countLike, callback, style } = props;
  ...
  const [countLikeText, setCountLikeText] = useState(0);
    ...
    <View style={[styles.container, style]} >
      ...
      <View style={{ flexDirection: 'row', width: '100%' }}>
        <View style={{ flexDirection: 'column', width: '85%'}}>
            <Text style={{ marginLeft: 8, fontWeight: 'bold', fontSize: 16,  alignSelf: 'flex-start' }}>{countLikeText}</Text>
        </View>
      </View>
    </View>
  );
